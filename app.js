...
const hasSupabaseConfig =
  ...
  /^https:\/\/[^\s]+\.supabase\.co$/i.test(SUPABASE_URL) &&
  SUPABASE_ANON_KEY.length > 20;

const supabase = hasSupabaseConfig && typeof createClient === "function"
...
async function boot(){
  // The complete UI remains usable in local preview mode until Supabase is configured.
  // Server-backed sign-in, shared memories and photo storage activate automatically
  // when valid values are placed in config.js.
  if(!supabase){
    if(hasSupabaseConfig && typeof createClient !== "function") {
      toast("Sign-in library did not load. Check your internet connection and refresh.");
    }
    state.user=null;
    state.space=null;
    state.memories.clear();
    appVisible();
    $("#settingsSpaceName").textContent="Local preview";
    $("#spaceId").textContent="Configure Supabase to enable shared sync";
    $("#memberCount").textContent="Local";
    renderAll();
    renderJournal();
    return;
  }
...
const LOCAL_MOODS_KEY = "our-little-world-moods-v3";
const LOCAL_THOUGHT_KEY = "our-little-world-thought-v3";
const LOCAL_THOUGHT_TIME_KEY = "our-little-world-thought-time-v3";

const defaultJournalThought = `Thank u for making this
...
function thoughtHtml(text){
  return String(text||"").trim().split(/\\n\\s*\\n/).filter(Boolean)
    .map(block=>`<p>${esc(block).replace(/\\n/g,"<br>")}</p>`).join("");
}

function renderJournal(){
  const text=localLoad(LOCAL_THOUGHT_KEY,defaultJournalThought);
  const time=localLoad(LOCAL_THOUGHT_TIME_KEY,new Date().toISOString());
  const textBox=$("#thoughtText"), timeBox=$("#thoughtTime");
  if(textBox) textBox.innerHTML=thoughtHtml(text);
  if(timeBox) timeBox.textContent=localTime(time);
}

function openJournalEditor(){
  const current=localLoad(LOCAL_THOUGHT_KEY,defaultJournalThought);
  $("#thoughtEditor").value=current;
...
function saveJournalThought(){
  const value=$("#thoughtEditor").value.trim();
  if(!value){toast("Write something first.");return}
  const now=new Date().toISOString();
  localSave(LOCAL_THOUGHT_KEY,value);
  localSave(LOCAL_THOUGHT_TIME_KEY,now);
  renderJournal();
  closeJournalEditor();
  toast("Thought saved ♡");
}
function todayLocalMoods(){
  const today=fmt(new Date());
  return localLoad(LOCAL_MOODS_KEY,[]).filter(x=>x.day===today);
}
function renderMood(){
  const todayMemory=state.memories.get(fmt(new Date()));
  const moods=todayLocalMoods().sort((a,b)=>new Date(a.time)-new Date(b.time));
  $$("#moodPicker button").forEach(b=>{
    b.classList.toggle("active",b.dataset.mood===todayMemory?.mood);
  });
  const timeline=$("#moodTimeline"), empty=$("#moodEmpty"), count=$("#moodCount");
  if(!timeline)return;
  if(count)count.textContent=moods.length;
  timeline.innerHTML=moods.map((m,i)=>`
    <div class="mood-timeline-item" style="--delay:${Math.min(i*45,300)}ms">
      <div class="mood-dot">${esc(m.emoji)}</div>
      <div class="mood-entry">
        <div><strong>${esc(m.label)}</strong><time>${esc(localTime(m.time))}</time></div>
        ${m.note?`<p>${esc(m.note)}</p>`:""}
      </div>
    </div>
  `).join("");
  if(empty)empty.hidden=moods.length>0;
}
function addLocalMood(emoji,label){
  const labels={"😊":"Happy","😌":"Calm","🥰":"Excited","💗":"Loved","😔":"Sad"};
  const now=new Date().toISOString();
  const moods=localLoad(LOCAL_MOODS_KEY,[]);
  moods.push({id:String(Date.now())+"-"+Math.random(),day:fmt(new Date()),emoji,label:labels[emoji]||label||"Mood",time:now});
  localSave(LOCAL_MOODS_KEY,moods);
  renderMood();
  toast(`${labels[emoji]||"Mood"} added at ${localTime(now)}.`);
}
...
async function saveMemory(){
  if(!supabase || !state.space || !state.user){toast("Shared saving is unavailable in preview mode.");return}
...
async function uploadPhotos(files){
  if(!supabase || !state.space || !state.user){toast("Photo uploads need Supabase configuration.");return}
...
async function removePhoto(id){
  if(!supabase){toast("Photo removal needs Supabase configuration.");return}
...
async function deleteMemory(){
  if(!supabase || !state.space || !state.user){toast("Shared deletion is unavailable in preview mode.");return}
...
async function createSpace(){
  if(!supabase){toast("Configure Supabase first to create a shared space.");return}
...
async function createInvite(){
  if(!supabase || !state.space){toast("Configure Supabase and create a shared space first.");return}
...
async function joinSpace(){
  if(!supabase){toast("Configure Supabase first to join a shared space.");return}
...
async function signIn(){
  if(!supabase){toast(hasSupabaseConfig ? "Supabase is not ready. Refresh and check your connection." : "Add your Supabase values in config.js first.");return}
  ...
  try{
    ...
    await loadSpace();
  }catch(err){ toast(err?.message || "Sign-in failed. Please try again."); }
  finally{ btn.disabled=false; btn.textContent="Sign in"; }
}
$("#signIn").addEventListener("click", signIn);
$("#authForm")?.addEventListener("submit", e=>{e.preventDefault(); signIn();});
$("#signUp").onclick=async()=>{
  if(!supabase){toast("Preview mode: add Supabase values in config.js to enable accounts.");return}
  const email=$("#email").value.trim();
  const password=$("#password").value;
...
$("#signOut").onclick=async(e)=>{
  if(!supabase){toast("You are viewing local preview mode.");return}
  e.preventDefault();
  e.stopPropagation();
...
state.dark=savedTheme ? savedTheme==="dark" : true;
...
$$("#moodPicker button").forEach(b=>b.onclick=()=>{
  addLocalMood(b.dataset.mood,b.querySelector("small")?.textContent||"Mood");
});

$("#editThoughtBtn")?.addEventListener("click",openJournalEditor);
$("#cancelThoughtBtn")?.addEventListener("click",closeJournalEditor);
$("#saveThoughtBtn")?.addEventListener("click",saveJournalThought);
renderJournal();
